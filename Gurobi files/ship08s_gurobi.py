"""
Industrial MPS Benchmark Instance: ship08s.mps using Gurobi.
Reads and constructs the Gurobi optimization model for ship08s.mps.
"""
import os
import time
import numpy as np
...
def build_ship08s_model(mps_path: str = None) -> gp.Model:
    """
    Constructs the Gurobi model for ship08s.mps.
    """
    if mps_path is None:
        base_dir = os.path.dirname(os.path.abspath(__file__))
        mps_path = os.path.join(base_dir, "ship08s.mps")

    model = gp.read(mps_path)
    model.ModelName = "ship08s"
    return model


if __name__ == "__main__":
    print("Building ship08s.mps model (Gurobi)...")
    t0 = time.perf_counter()
    m = build_ship08s_model()
    m.optimize()
