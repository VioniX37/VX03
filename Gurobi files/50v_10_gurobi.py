"""
MIPLIB Benchmark Instance: 50v-10 using Gurobi.
Reads and constructs the Gurobi optimization model for 50v-10.mps.
"""
import os
import time
import numpy as np
...
def build_50v_10_model(mps_path: str = None) -> gp.Model:
    """
    Constructs the Gurobi model for 50v-10.mps.
    """
    if mps_path is None:
        base_dir = os.path.dirname(os.path.abspath(__file__))
        mps_path = os.path.join(base_dir, "50v-10.mps")

    model = gp.read(mps_path)
    model.ModelName = "50v-10"
    return model


if __name__ == "__main__":
    print("Building 50v-10 MIPLIB model (Gurobi)...")
    t0 = time.perf_counter()
    m = build_50v_10_model()
    m.optimize()
