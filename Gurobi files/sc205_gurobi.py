"""
Industrial MPS Benchmark Instance: sc205.mps using Gurobi.
Reads and constructs the Gurobi optimization model for sc205.mps.
"""
import os
import time
import numpy as np
...
def build_sc205_model(mps_path: str = None) -> gp.Model:
    """
    Constructs the Gurobi model for sc205.mps.
    """
    if mps_path is None:
        base_dir = os.path.dirname(os.path.abspath(__file__))
        mps_path = os.path.join(base_dir, "sc205.mps")

    model = gp.read(mps_path)
    model.ModelName = "sc205"
    return model


if __name__ == "__main__":
    print("Building sc205.mps model (Gurobi)...")
