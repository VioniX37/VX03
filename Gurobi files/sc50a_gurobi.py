"""
Industrial MPS Benchmark Instance: sc50a.mps using Gurobi.
Reads and constructs the Gurobi optimization model for sc50a.mps.
"""
import os
import time
import numpy as np
...
def build_sc50a_model(mps_path: str = None) -> gp.Model:
    """
    Constructs the Gurobi model for sc50a.mps.
    """
    if mps_path is None:
        base_dir = os.path.dirname(os.path.abspath(__file__))
        mps_path = os.path.join(base_dir, "sc50a.mps")

    model = gp.read(mps_path)
    model.ModelName = "sc50a"
    return model


if __name__ == "__main__":
    print("Building sc50a.mps model (Gurobi)...")
